...
class CobDataLoader(Dataset):
    def __init__(self,
        ...
        # self.dl = BSDS_Dataset(root_imgs, root_segs, split=split)
    ...
    def prepare_all(self):
        if not (os.path.exists(self.or_cntr_path)):
            ...
            # dl = BSDS_Dataset(self.root_imgs, self.root_segs)
    ...
    def __getitem__(self, idx):
        sample = self.dl[idx] # {image:'',labels:'',base_name:'2008_000008'}
...
if __name__ == "__main__":
    ...
    for i in range(x.shape[0]):
        plt.plot(x[i], y[i], '.', color=plt.cm.RdYlBu(bins[i] / 8))
