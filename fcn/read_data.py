...
def read_voc_images(voc_dir, is_train=True):
    ...
    with open(txt_fname, 'r') as f:
        images = f.read().split()
    # print(images) ['2007_000032', '2007_000039', '2007_000063', '2
    features, labels = [], []
...
voc_dir = "D:\work\dataset\VOC2012"
...
# 列举RGB颜色值和类名（每一个标号对应pixel的RGB值） 21个类别
VOC_COLORMAP = [[0, 0, 0], # 黑--background
                [128, 0, 0], # 红-- aeroplane
                [0, 128, 0], # 绿-- bicycle
                [128, 128, 0],
...
def voc_colormap2label():
    ...
    for i, colormap in enumerate(VOC_COLORMAP):
        print(i,colormap)
        print( (colormap[0] * 256 + colormap[1]) * 256 + colormap[2],i)
        colormap2label[
...
# 自定义语义分割数据集类
# 做小批量训练的时候，需要把图片做成一样大，所以需要crop，不能随意resize
class VOCSegDataset(torch.utils.data.Dataset):
    """一个用于加载VOC数据集的自定义数据集"""

    def __init__(self, is_train, crop_size, voc_dir):
        # 之后想用ImageNet上的pre-train model
