...
def single_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, image in enumerate(test_loader):
        ...
        _, _, H, W = image.shape
        results = model(image)
        all_res = torch.zeros((len(results), 1, H, W)) #6，1，h,w
        for i in range(len(results)):
            all_res[i, 0, :, :] = results[i]
        filename = osp.splitext(test_list[idx])[0] # 数字
        torchvision.utils.save_image(1 - all_res, osp.join(save_dir, '%s.jpg' % filename)) # all_res 白的程度，1-all 即黑的程度
        # results为len=6的list，最后一个-1是fusion_result,即融合结果。

        # 0-5都输出一下
        for i in range(len(results)):
            fuse_res = torch.squeeze(results[i].detach()).cpu().numpy()
            fuse_res = ((1 - fuse_res) * 255).astype(np.uint8)
            cv2.imwrite(osp.join(save_dir, '%s_result' % filename+'{}.png'.format(i)), fuse_res)
        # fuse_res = torch.squeeze(results[-1].detach()).cpu().numpy()
        # fuse_res = ((1 - fuse_res) * 255).astype(np.uint8)
        # cv2.imwrite(osp.join(save_dir, '%s_result.png' % filename), fuse_res)
...
if __name__ == '__main__':
    ...
    test_loader = DataLoader(dataset=test_dataset, batch_size=1, num_workers=1, drop_last=False, shuffle=False)
    ...
    checkpoint = torch.load("./bsds500_pascal_model.pth", map_location="cpu")
