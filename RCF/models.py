...
class RCF(nn.Module):
    ...
    def forward(self, x):
        ...
        weight_deconv2 =  make_bilinear_weights(4, 1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        weight_deconv3 =  make_bilinear_weights(8, 1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        weight_deconv4 =  make_bilinear_weights(16, 1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        weight_deconv5 =  make_bilinear_weights(32, 1).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
